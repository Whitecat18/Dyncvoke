// dyncvoke - Manual PE Mapping Module
// @5mukx
...
pub fn manually_map_module(
    ...
    unsafe {
        ...
        let _r = dyncvoke_core::close_handle(handle);
        ...
        while next_reloc_table_block != 0 {
            ...
            for i in 0..reloc_count {
                ...
                if reloc_type != 0 {
                    ...
                    if reloc_type == 0x3 {
                        // IMAGE_REL_BASED_HIGHLOW: 32-bit; target must be 4-byte aligned
                        if patch_addr % 4 == 0 {
                        ...
                        // IMAGE_REL_BASED_DIR64 (0xA) and others: 64-bit; target must be 8-byte aligned
                        // Skip misaligned entries to avoid misaligned pointer dereference (e.g. ntdll on some builds)
                        if patch_addr % 8 == 0 {
    ...
    unsafe {
        ...
        while image_import_descriptor.Name != 0 {
            ...
            if dll_name == "" {
                ...
                if (version >= 10.to_string()
                    && (dll_name.starts_with("api-") || dll_name.starts_with("ext-")))
...
pub fn run_tls_callbacks(pe_info: &PeMetadata, image_ptr: *mut c_void) {
    unsafe {
        ...
        if pe_info.opt_header_64.number_of_rva_and_sizes >= 10 {
            let address: *mut u8 = (image_ptr as usize
...
pub fn map_to_section(module_path: &str) -> Result<(PeManualMap, HANDLE), String> {
    unsafe {
        ...
        let o_name: UNICODE_STRING = std::mem::zeroed();
        let object_name: *mut UNICODE_STRING = std::mem::transmute(&o_name);
        dyncvoke_core::rtl_init_unicode_string(object_name, module_path_utf16.as_ptr());
