...
pub const INVALID_HANDLE_VALUE: HANDLE = std::ptr::null_mut();

#[cfg(all(target_arch = "x86_64", not(feature = "syscall")))]
use data::{
    ApiSetNamespace, ApiSetNamespaceEntry, ApiSetValueEntry, ClientId, EntryPoint,
    ExceptionHandleFunction, ExceptionPointers, LptopLevelExceptionFilter,
    NtAllocateVirtualMemoryArgs, NtCreateThreadExArgs, NtOpenProcessArgs,
    NtProtectVirtualMemoryArgs, NtWriteVirtualMemoryArgs, PeMetadata, PsAttributeList,
    PsCreateInfo, DLL_PROCESS_ATTACH, EAT, MAX_PATH, MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READ,
    PAGE_EXECUTE_READWRITE, PAGE_READONLY, PAGE_READWRITE, PROCESS_QUERY_LIMITED_INFORMATION,
    PVOID, TLS_OUT_OF_INDEXES,
};
...
use data::{
    ...
    NtAllocateVirtualMemoryArgs, NtCreateThreadExArgs, NtOpenProcessArgs,
    NtProtectVirtualMemoryArgs, NtWriteVirtualMemoryArgs, PeMetadata, PsAttributeList,
    PsCreateInfo, DLL_PROCESS_ATTACH, EAT, MAX_PATH, PAGE_EXECUTE_READWRITE, PAGE_READONLY,
    PROCESS_QUERY_LIMITED_INFORMATION, PVOID, TLS_OUT_OF_INDEXES,
};
#[cfg(target_arch = "x86")]
use data::{
    ApiSetNamespace, ApiSetNamespaceEntry, ApiSetValueEntry, ClientId, EntryPoint,
    LptopLevelExceptionFilter, PeMetadata, PsAttributeList, PsCreateInfo, DLL_PROCESS_ATTACH, EAT,
    MAX_PATH, PAGE_EXECUTE_READWRITE, PVOID, TLS_OUT_OF_INDEXES,
};
#[cfg(target_arch = "x86_64")]
...
pub fn get_last_error() -> u32 {
    // Use windows_sys directly to avoid infinite recursion
    // (calling get_module_base_address would cause recursion)
